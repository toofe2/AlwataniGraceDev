...
static BOOL AGDLooksLikeName(NSString *candidate, NSArray<NSString *> *names) {
    NSString *c = candidate.lowercaseString;
    if (!c.length) return NO;
    for (NSString *name in names) if ([c containsString:name.lowercaseString]) return YES;
    return NO;
}
...
static NSString *AGDStorageDiagnostic(void) {
    NSArray<NSString *> *tokenNames = @[@"user_access_token", @"access_token", @"accessToken"];
    NSArray<NSString *> *subNames = @[@"selected_subscription_id", @"subscriptionId", @"subscription_id"];
    NSMutableArray<NSString *> *lines = [NSMutableArray array];

    NSDictionary *defaults = NSUserDefaults.standardUserDefaults.dictionaryRepresentation;
    BOOL tokenDefault = NO, subDefault = NO;
    for (NSString *key in defaults.allKeys) {
        if (AGDLooksLikeName(key, tokenNames)) tokenDefault = YES;
        if (AGDLooksLikeName(key, subNames)) subDefault = YES;
    }
    [lines addObject:[NSString stringWithFormat:@"NSUserDefaults token key: %@", tokenDefault ? @"FOUND" : @"not found"]];
    [lines addObject:[NSString stringWithFormat:@"NSUserDefaults subscription key: %@", subDefault ? @"FOUND" : @"not found"]];

    BOOL tokenKC = NO, subKC = NO;
    NSMutableArray<NSString *> *matchedAccounts = [NSMutableArray array];
    for (NSDictionary *item in AGDAllKeychainItems()) {
        ...
        if (AGDLooksLikeName(account, tokenNames) || AGDLooksLikeName(service, tokenNames)) tokenKC = YES;
        if (AGDLooksLikeName(account, subNames) || AGDLooksLikeName(service, subNames)) subKC = YES;
        if ((AGDLooksLikeName(account, tokenNames) || AGDLooksLikeName(account, subNames)) && account.length) [matchedAccounts addObject:account];
    }
    [lines addObject:[NSString stringWithFormat:@"Keychain token entry: %@", tokenKC ? @"FOUND" : @"not found"]];
    [lines addObject:[NSString stringWithFormat:@"Keychain subscription entry: %@", subKC ? @"FOUND" : @"not found"]];
    if (matchedAccounts.count) [lines addObject:[NSString stringWithFormat:@"Matched key names: %@", [matchedAccounts componentsJoinedByString:@", "]]];
    [lines addObject:@"No secret/token values are displayed or exported."];
...
static void AGDShowPanel(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *root = AGDTopViewController();
        if (!root) return;
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Grace Days — DEV" message:@"MK injection active. Storage diagnostics can identify where the app keeps its session metadata without exposing secret values." preferredStyle:UIAlertControllerStyleAlert];
